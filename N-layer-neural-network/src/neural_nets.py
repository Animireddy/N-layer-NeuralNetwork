...
#     print(W1.shape)
    ...
    # Output scores.
    scores = fc2_activation  #N*10 size
    ...
    #substract every value by max value in each row
    shift_scores = scores - np.max(scores, axis=1)[..., np.newaxis]

    # Calculate softmax scores.
    #it computes e^z/sigma(e^z) for every row
    softmax_scores = np.exp(shift_scores) / np.sum(np.exp(shift_scores), axis=1)[..., np.newaxis] #N*d
#     softmax_scores = 1 / (1 + np.exp(shift_scores))

    # Calculate our cross entropy Loss.
    #y=(indices) ith index element index[i] array => ith ele pick
#     print(shift_scores.T.shape) #100 samples => 10 prob's
#     print(y.shape) #100*1 denoting o/p's #100 samples o/p
# choosing that shift score with maximum value out of 10    
    correct_class_scores = np.choose(y, shift_scores.T) # Size N vector
#     print(correct_class_scores.shape)
#     print(correct_class_scores)
    loss = -correct_class_scores + np.log(np.sum(np.exp(shift_scores), axis=1)) #N*1
    loss = np.sum(loss)
  ...
  def train(self, X, y, X_val, y_val,
    ...
    # Use SGD(Stochastic Gradient Descent) to optimize the parameters in self.model
    loss_history = []
    train_acc_history = []
    val_acc_history = []
    ...
    for it in range(num_iters):
      ...
      #forward and backward propogation happens in loss function
